...
const signup = async(req,res)=>{
     try
     {
        ...
        if(user)
        {
            res.status(400).send({status:false,message:"User with same email already exist",error:true});
        }
        user = await User.create(req.body);
        const token=generateToken(user);
        console.log("token 26 ",token,user);
        res.status(201).send({status:true,token:token,message:"User signed in successfully",error:false});
     }
     catch(e)
     {
        res.status(500).send({status:false,message:"User could not be signed up, some error occurred",error:e.message});
